...
besmerged_clean <- besmerged_clean %>%
...
besmerged_sid_overtime <- besmerged_des %>%
  group_by(startdate) %>%
  summarize(
    `Left SId` = survey_mean(lr_sid == "Left", na.rm = TRUE) * 100,
    `Centre SId` = survey_mean(lr_sid == "Centre", na.rm = TRUE) * 100,
    `Right SId` = survey_mean(lr_sid == "Right", na.rm = TRUE) * 100,
    `Don't Know SId` = survey_mean(lr_sid == "DK", na.rm = TRUE) * 100,
    wave = first(wave)
  )
...
besmerged_sid_overtime_long <- besmerged_sid_overtime %>%
  pivot_longer(cols = c(`Left SId`, `Centre SId`, `Right SId`, `Don't Know SId`), names_to = "ideology", values_to = "p")

besmerged_sid_overtime_long <- besmerged_sid_overtime_long %>%
  mutate(p_round = round(p, 1))

besmerged_sid_overtime_long <- besmerged_sid_overtime_long %>%
  mutate(lastpoint = ifelse(wave == "W29", p_round, "")) 
...
besmerged_sid_overtime_last_points <- besmerged_sid_overtime_long %>%
  group_by(ideology) %>%
  filter(startdate == max(startdate))

png("bespanel/srid_besv2.png", width = 10*ppi, height = 5*ppi, res = ppi)

p1 <- ggplot(besmerged_sid_overtime_long, aes(x = startdate, y = p,
                       color = ideology, group = ideology)) + 
  geom_line(size = 1) +
  geom_point(size = 2) +
  labs(x = "BESIP wave start date", 
  ...
  scale_color_manual(values = c("#228833",
                                "#BBBBBB",
                                "#EE6677",
                                "#4477AA")) +
  ...
  theme(axis.title.x = element_text(margin = margin(t = 12), size = 12),
        ...
        plot.title = element_text(margin = margin(12,0,12,0),
                                  size = 14,
                                  face = "bold")) +
  geom_text(aes(x = startdate + 50, label = lastpoint), size = 4, hjust = 0,
            show.legend = FALSE)
...
besmerged_clean_test <- besmerged_clean %>%
  filter(wave == "W29")
...
besmerged_clean_test %>%
  tab_cells(lr_sid) %>%
  tab_cols(total(label = "Total %")) %>%
  tab_weight(wt) %>%
  tab_stat_cpct(total_label = "Number of respondents") %>%
  tab_pivot() %>%
  tab_caption("UK Left-Right Values by Wave)")
